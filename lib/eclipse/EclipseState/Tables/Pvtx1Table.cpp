...
namespace Opm {
    ...
    Pvtx1Table::Pvtx1Table(const std::string& columnName) :
    ...
    */
    ...
    void Pvtx1Table::init( const DeckKeyword& keyword, size_t tableIdx) {
...

        {
            ...
            for (size_t  rowIdx = range.first; rowIdx < range.second; rowIdx++) {
                ...
                const auto& dataItem = deckRecord.getItem(0);
                //m_underSaturatedTables.emplace_back( this->m_underSaturatedSchema, dataItem );
                m_rsTables.emplace_back( this->m_rsSchema, dataItem );
            }
            ...
            for (size_t sat_index = 0; sat_index < size(); sat_index++) {
                ...
                const auto& rsTable = getRsTable( sat_index );
                std::vector<double> row(4);
                //row[0] = m_outerColumn[sat_index];
                for (size_t col_index = 0; col_index < m_rsSchema.size(); col_index++)
                    row[col_index] = rsTable.get( col_index , 0 );
    ...
    /*
    double Pvtx1Table::evaluate(const std::string& column, double outerArg, double innerArg) const
    {
    ...
    */
    ...
    {
        TableIndex outerIndex = m_outerColumn.lookup( outerArg );
        const auto& rsTable1 = getRsTable( outerIndex.getIndex1( ) );
        double weight1 = outerIndex.getWeight1( );
        double value = weight1 * rsTable1.evaluate( column , innerArg );

        if (weight1 < 1) {
            const auto& rsTable2 = getRsTable( outerIndex.getIndex2( ) );
            double weight2 = outerIndex.getWeight2( );

            value += weight2 * rsTable2.evaluate( column , innerArg );
        }

        return value;
    }

    const SimpleTable& Pvtx1Table::getSaturatedTable() const {
        return this->m_saturatedTable;
    }

    //sogo
  /*
    const SimpleTable& Pvtx1Table::getRsTable() const {
        return this->m_rsTables;
    }
   */
    //sogo
    const SimpleTable& Pvtx1Table::getRsTable(size_t tableNumber) const {
        if (tableNumber >= size())
            throw std::invalid_argument("Invalid table number: " + std::to_string( tableNumber) + " max: " + std::to_string( size() - 1 ));
        return m_rsTables[ tableNumber ];
    }


    /*
    const SimpleTable& Pvtx1Table::getUnderSaturatedTable(size_t tableNumber) const {
        if (tableNumber >= size())
            throw std::invalid_argument("Invalid table number: " + std::to_string( tableNumber) + " max: " + std::to_string( size() - 1 ));
    ...
    */
    /*
    std::vector< SimpleTable >::const_iterator Pvtx1Table::begin() const {
        return m_underSaturatedTables.begin();
    }

    std::vector< SimpleTable >::const_iterator Pvtx1Table::end() const {
        return m_underSaturatedTables.end();
    }
    */
    std::vector< SimpleTable >::const_iterator Pvtx1Table::begin() const {
        return m_rsTables.begin();
    }

    std::vector< SimpleTable >::const_iterator Pvtx1Table::end() const {
        return m_rsTables.end();
    }

    size_t Pvtx1Table::size() const
    {
        return m_outerColumn.size();
    }


    size_t Pvtx1Table::numTables( const DeckKeyword& keyword )
    {
        auto ranges = recordRanges(keyword);
        return ranges.size();
    }
    ...
    std::vector<std::pair<size_t , size_t> > Pvtx1Table::recordRanges( const DeckKeyword& keyword ) {
    ...
    double Pvtx1Table::getArgValue(size_t index) const {
